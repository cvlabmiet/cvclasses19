...
{
    cv::Mat image;
    auto res = split_and_merge(image, 1, 1);
    REQUIRE(res.empty());
}
...
TEST_CASE("constant image", "[split_and_merge]")
{
    ...
    const auto res = split_and_merge(image, 1, 1);
...
TEST_CASE("simple regions", "[split_and_merge]")
{
    SECTION("2x2")
    {
        const cv::Mat reference = (cv::Mat_<uint8_t>(2, 2) << 1, 1, 1, 1);
        cv::Mat image = (cv::Mat_<uint8_t>(2, 2) << 0, 1, 2, 3);
        auto res = split_and_merge(image, 10, 1);
        REQUIRE(image.size() == res.size());
        REQUIRE(image.type() == res.type());
        REQUIRE(0 == cv::countNonZero(reference - res));

        res = split_and_merge(image, 0, 1);
    ...
    SECTION("3x3")
    {
        ...
        cv::Mat image = (cv::Mat_<uint8_t>(3, 3) <<
            ...
            3, 4, 5,
            6, 7, 8);
        auto res = split_and_merge(image, 20, 1);
        REQUIRE(0 == cv::countNonZero(reference - res));
    }
}
...
TEST_CASE("compex regions", "[split_and_merge]")
{
    SECTION("2x2")
    {
        ...
        auto res = split_and_merge(image, 5, 1);
        REQUIRE(0 == cv::countNonZero(reference - res));
    }
    ...
    SECTION("4x4")
    {
        ...
        const cv::Mat reference = (cv::Mat_<uint8_t>(4, 4) <<
            ...
            12,  12,  90,  90,
            12,   1,   1,   1);
        auto res = split_and_merge(image, 3, 1);
        REQUIRE(0 == cv::countNonZero(reference - res));
    }
}
