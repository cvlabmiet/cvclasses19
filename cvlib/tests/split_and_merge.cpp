...
{
    cv::Mat image, res;
    split_and_merge(image, cv::Mat(), res, 1, 0, 0, 1);
    REQUIRE(res.empty());
}
...
TEST_CASE("simple regions", "[split_and_merge]")
{
    SECTION("2x2")
    {
        ...
        cv::Mat res, image = (cv::Mat_<char>(2, 2) << 0, 1, 2, 3);
        split_and_merge(image, cv::Mat(), res, 2, 0, 0, 1);
    ...
    SECTION("3x3")
    {
        ...
        cv::Mat res, image = (cv::Mat_<char>(3, 3) << 0, 1, 2, 3, 4, 5, 6, 7, 8);
        split_and_merge(image, cv::Mat(), res, 3, 0, 0, 1);
        REQUIRE(0 == cv::countNonZero(reference - res));
    }
}
...
TEST_CASE("complex regions", "[split_and_merge]")
{
    SECTION("2x2")
    {
        ...
        cv::Mat res, image = (cv::Mat_<char>(2, 2) << 0, 99, 100, 101);
        split_and_merge(image, cv::Mat(), res, 40, 0, 1, 1);
        REQUIRE(0 == cv::countNonZero(reference - res));

        split_and_merge(image, cv::Mat(), res, 40, 0, 0, 1);
        REQUIRE(0 == cv::countNonZero(image - res));
    }
    ...
    SECTION("3x3")
    {
        ...
        cv::Mat res, image = (cv::Mat_<char>(3, 3) << 0, 0, 0, 0, 98, 99, 0, 100, 101);
        split_and_merge(image, cv::Mat(), res, 40, 0, 1, 1);
        REQUIRE(0 == cv::countNonZero(reference - res));
    }
    ...
    SECTION("4x4")
    {
        ...
        cv::Mat res, image = (cv::Mat_<char>(4, 4) << 0, 0, 0, 0, 0, 0, 97, 98, 48, 49, 99, 100, 50, 51, 101, 102);
        split_and_merge(image, cv::Mat(), res, 40, 0, 1, 2);
        REQUIRE(0 == cv::countNonZero(reference - res));
    }
}
