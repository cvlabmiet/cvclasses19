...
TEST_CASE("compex regions", "[split_and_merge]")
{
    ...
    SECTION("4x4")
    {
        const cv::Mat reference = (cv::Mat_<char>(4, 4) << 3, 3, 3, 3, 3, 3, 3, 3, 20, 20, 20, 20, 40, 40, 40, 40);
        cv::Mat image = (cv::Mat_<char>(4, 4) << 1, 2, 3, 2, 4, 5, 3, 4, 21, 19, 22, 18, 41, 39, 40, 40);
        auto res = split_and_merge(image, 10);
        REQUIRE(image.size() == res.size());
        REQUIRE(image.type() == res.type());
        REQUIRE(0 == cv::countNonZero(reference - res));
    ...
    SECTION("4x4 Extra")
    {
        const cv::Mat reference = (cv::Mat_<char>(4, 4) << 0, 0, 100, 100, 0, 100, 100, 100, 0, 0, 0, 0, 0, 0, 0, 0);

        cv::Mat image = (cv::Mat_<char>(4, 4) << 0, 0, 102, 100, 0, 100, 102, 100, 0, 0, 0, 0, 0, 0, 0, 0);
        auto res = split_and_merge(image, 10);
        REQUIRE(image.size() == res.size());
        REQUIRE(image.type() == res.type());
